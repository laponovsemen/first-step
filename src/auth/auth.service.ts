...
export class AuthService implements OnModuleInit{
  ...
  async signIn(user : User, ip : string, title : string, deviceId : ObjectId) {
    ...
    return {
      access_token: await this.jwtService.signAsync(payload, {expiresIn: '5m', secret :jwtConstants.secretForAccess}),
      refresh_token: await this.jwtService.signAsync(payload, {expiresIn: '10m', secret :jwtConstants.secretForRefresh}),
    };
  }
  ...
  async refreshToken(refreshToken: string) {
    ...
    const newAccessToken = await this.jwtService.signAsync(payload, {expiresIn: '5m',secret :jwtConstants.secretForAccess})
    const newRefreshToken = await this.jwtService.signAsync(payload, {expiresIn: '10m',secret :jwtConstants.secretForRefresh})
